...
def forward_fill_pipeline(
    ...
    for name, group in grouped:
        ...
        patient_x = []
        patient_y = []
        patient_x_mask = np.isnan(sorted_group[demographic_features + labtest_features].values).astype(int)
        ...
        for f in require_impute_features:
            ...
            _, feature_mask = fill_missing_value(sorted_group[f].values, to_fill_value)
            all_features = demographic_features + labtest_features
            # if f in all_features:
            #     patient_x_mask[:, all_features.index(f)] = feature_mask
        ...
        all_x_mask.append(patient_x_mask)
        all_y.append(patient_y)
        all_pid.append(name)
    return all_x, all_x_mask, all_y, all_pid


def fill_missing_values(matrix):
    # 转换为DataFrame以使用fillna函数
    df = pd.DataFrame(matrix)
    # 创建填充mask
    fill_mask = np.isnan(matrix).astype(int)
    # 向前填充
    df = df.ffill(axis=0)
    # 向后填充
    df = df.bfill(axis=0)
    return df.values, fill_mask
